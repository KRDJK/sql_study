...
CREATE TABLE CHAR_COMPARE (
    sn CHAR(10), -- CHAR 타입은 뭐가 어찌됐건 무조건 (BYTE) 수만큼의 글자를 만든다.
    char_4 CHAR(4),
    char_6 CHAR(6)
);
...
FROM char_compare
WHERE sn = '101'
    AND char_4 = char_6 -- CHAR 타입에서는 공백이 있든 없든 안에 든 텍스트만 같으면 같다고 본다.
;
...
FROM char_compare
WHERE sn = '102'
    AND char_4 > char_6 -- 아스키 코드로 대소 비교한다.
;
...
WHERE sn = '101'
    AND char_4 = varchar_6 -- VARCHAR2는 글자수가 애초에 다른데 어케 같냐고 하는 것.
;
...
FROM varchar_compare
WHERE sn = '101'
    AND char_4 = 'SQLD         ' -- 비교대상이 char니까 상수도 char로 취급해서 비교!
;
...
FROM varchar_compare
WHERE sn = '101'
    AND varchar_6 = 'SQLD' -- 비교대상이 varchar니까 상수도 varchar 취급되어 비교함.
;


-- # 단일행 함수 : 각 행 하나하나에 함수를 적용하겠다는 뜻
-- ## 문자열 함수
SELECT 
...
SELECT 
    ...
    LENGTH('HELLO WORLD'),         -- 문자열의 길이, 공백도 세줌
    TRIM('    HI   ')              -- 좌우 공백 제거
FROM dual;
